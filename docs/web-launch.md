...
Version 1.2.6 registers `anty://launch/<remoteId>?background=1` on macOS and Windows.
...
It must not use loss of browser focus as proof of installation or launch success.
A timeout means unconfirmed, not uninstalled. The browser may ask permission to
open the external application; the user must allow it. Version 1.2.5 does not
register this profile-launch protocol and needs an update.
...
notarization setup. Do not publish local test artifacts as a notarized release.
