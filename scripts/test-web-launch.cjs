...
test('cold-start requests wait for ready, deduplicate and launch without showing UI', async () => {
  ...
  assert.deepEqual(h.events.map(e => e[0]), ['sync', 'launch', 'ack']);
  assert.equal(h.events[1][2], 'hidden-window'); assert.equal(h.controller.hasPending(), false);
});
test('already running profiles acknowledge without launching twice', async () => {
  ...
  assert.deepEqual(h.events.map(e => e[0]), ['sync', 'ack']);
});
test('login, missing profile, remote lock, sync and launch failures never acknowledge success', async () => {
  for (const options of [{ loggedIn: false }, { profile: null }, { profile: { id: 4, status: 'running' } }, { syncOk: false }, { success: false }]) {
...
test('Electron cold starts hide manager for Windows argv and macOS open-url; normal start shows it', async () => {
  for (const mode of ['argv', 'mac-url', 'normal']) {
    let resolveReady, pending = false;
    const listeners = {}, windows = [];
    const app = { requestSingleInstanceLock: () => true, on(name, fn) { listeners[name] = fn; },
      whenReady: () => new Promise(resolve => { resolveReady = resolve; }), setAsDefaultProtocolClient() {} };
    class BrowserWindow {
    ...
    const modules = {
      electron: { app, BrowserWindow, ipcMain: { on() {}, removeAllListeners() {} }, nativeImage: {}, dialog: {} },
    ...
    resolveReady(); await new Promise(resolve => setImmediate(resolve));
    assert.equal(windows[0].options.show, mode === 'normal');
    if (mode !== 'normal') { listeners.activate(); assert.equal(windows[0].shown, undefined); }
  }
});
